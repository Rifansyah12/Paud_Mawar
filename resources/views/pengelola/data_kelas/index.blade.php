...
<div class="mb-3 d-flex justify-content-between align-items-center">
                ...
                @forelse ($kelas as $index => $k)
                    <tr>
                        ...
                        <td class="text-center">
                            <!-- Lihat Detail -->
                            <button class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#modalDetail{{ $k->id }}">Detail</button>

                            <!-- Edit -->
                            <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#modalEdit{{ $k->id }}">Edit</button>

                            <!-- Hapus -->
                            <form action="{{ route('kelas.destroy', $k->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Yakin ingin menghapus kelas ini?')">
                                @csrf
                                @method('DELETE')
                            ...
                            <form action="{{ route('kelas.update', $k->id) }}" method="POST">
                                ...
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="modalEditLabel{{ $k->id }}">Edit Kelas</h5>
    ...
    {{-- Modal Detail --}}
                    <div class="modal fade" id="modalDetail{{ $k->id }}" tabindex="-1" aria-labelledby="modalDetailLabel{{ $k->id }}" aria-hidden="true">
                        <div class="modal-dialog modal-lg"> {{-- pakai modal-lg agar cukup lebar --}}
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="modalDetailLabel{{ $k->id }}">Detail Kelas: {{ $k->nama_kelas }}</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Tutup"></button>
                                </div>
                                <div class="modal-body">
                                    ...
                                    <!-- Tombol Unduh PDF -->
                                    <a href="{{ route('kelas.export.siswa', $k->id) }}" class="btn btn-sm btn-success mb-3" target="_blank">
                                        Unduh Data Siswa
                                    </a>
